				...
				.forEach(function (rr) {
					if (rr.type === 'SRV') {
						...
						if (Array.isArray(data)) {
							service.txt = data.map(item => txt.decode(Buffer.concat([Buffer.from([item.length]), item])));
						} else {
							service.txt = data.length > 0 && data[0] !== 0 ? txt.decode(Buffer.concat([Buffer.from([data.length]), data])) : {}
						}	
					}
				});
