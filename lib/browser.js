...
function Browser(mdns, opts, onup) {
	...
	this._serviceMap = {};
	this._txtOptions = opts.txtOptions || opts.txt || {};
	...
	if (types.length) {
		types.filter(type => Boolean(type)).forEach(type => {
			if (typeof type === 'string') {
				this._names.push(serviceName.stringify({ name: type, protocol: opts.protocol }));
				...
				names.forEach(name => {
					...
					subtypes.forEach(subtype => {
						this._names.push(serviceName.stringify({ name: name, protocol: opts.protocol, subtypes: subtype }));
					});
				});
			}
		});
...
Browser.prototype.start = function () {
	...
	const nameInMap = function (recordName) {
		return self._names.some(function (name) {
			return dnsEqual(name, recordName);
		})
	};
	...
	this._onresponse = function (packet, rinfo) {
		if (self._wildcard) {
			packet.answers.forEach(function (answer) {
				if (answer.type !== 'PTR' || !nameInMap(answer.name) || answer.name in nameMap) return;
		...
		Object.keys(nameMap).forEach(function (name) {
			...
			const matches = buildServicesFor(name, packet, self._txtOptions, rinfo);
...
function buildServicesFor(name, packet, txtOptions, referer) {
	...
	return records
		...
		.map(function (ptr) {
			...
			records
				.filter(function (rr) {
					return rr.type === 'PTR' && dnsEqual(rr.data, ptr.data)
				})
				.forEach(function (rr) {
					const subtypes = serviceName.parse(rr.name).subtypes
					if (subtypes.length) {
						service.subtypes.push(subtypes.join('.'))
					}
				})
			...
			records
				...
				.forEach(function (rr) {
					if (rr.type === 'SRV') {
						service.fqdn = Array.isArray(rr.name) ? rr.name.join('.') : rr.name;

						const parts = serviceName.parse(service.fqdn);
						service.name = parts.instance;
						service.host = rr.data.target;
						service.referer = referer;
						service.port = rr.data.port;
						service.type = parts.name;
						service.protocol = parts.protocol;
						...
						service.txt = data.reduce(function (res, item) {
							if(item.length) {
								const idx = item.indexOf("=");
								const key = item.slice(0, ~idx ? idx : item.length ).toString();
								const value = item.slice(key.length + 1);
								if (key && !(key in res)) res[key] = txtOptions.binary ? value : value.toString();
							}
							return res;
						}, {});
					}
