...
Server.prototype._recordsFor = function (name, type) {
	...
	return this.registry[type].filter(function (record) {
		// exclude service listing for non-browse query
		if (type === "PTR" && name !== WILDCARD && record.name === WILDCARD)
			return false		// exclude service listing for non-browse query

		// include service listing for browse query
		if (type === "PTR" && name === WILDCARD)
			return record.name === WILDCARD
